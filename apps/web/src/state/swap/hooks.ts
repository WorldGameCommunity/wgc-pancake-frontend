...
import { USDT, WGC, USDC } from '@pancakeswap/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, WGC[chainId]?.address ?? USDC[chainId]?.address, USDT[chainId]?.address ?? USDC[chainId]?.address)
